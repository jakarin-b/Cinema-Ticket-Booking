...
func Router(cfg config.Config, handlers *Handlers, authService *appauth.Service, redisClient *redis.Client) *gin.Engine {
	...
	v1 := r.Group("/api/v1")
	{
		...
		authenticatedAuth.Use(RequireAuth(authService))
		...
		protected := v1.Group("")
		protected.Use(RequireAuth(authService))
		...
		admin.Use(RequireAuth(authService), RequireAdmin())
